...
async def generate_from_chunk(
    ...
    """Generate a QA pair and use semantic retrieval to get the true expected_retrieval_ids."""
    async with semaphore:
        try:
            ...
            # Use semantic retrieval (same as agent) to get expected_retrieval_ids
            def _retrieve():
                ...
                return results["ids"][0]  # top-3 document IDs
            
            expected_ids = await loop.run_in_executor(None, _retrieve)
            
            return {
                "question":               question,
                "expected_answer":        parsed["expected_answer"],
                "expected_retrieval_ids": expected_ids,  # Use semantic retrieval results
                "metadata": {
...
async def main():
    ...
    # Initialize embedding model (same as agent uses)
    ...
    tasks     = [generate_from_chunk(nvidia_client, cid, doc, semaphore, embed_model, collection)
